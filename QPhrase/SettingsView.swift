...
struct PromptDetailView: View {
    ...
    // Get the current prompt from the manager to ensure we have the latest state
    private var currentPrompt: Prompt {
        promptManager.prompts.first(where: { $0.id == prompt.id }) ?? prompt
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(alignment: .center, spacing: 12) {
                PromptIconView(
                    iconName: currentPrompt.icon,
                    color: IconColorMapper.color(for: currentPrompt.icon),
                    size: 36
                )

                Text(currentPrompt.name)
                    .font(.title2)
                    .fontWeight(.semibold)
                    .foregroundColor(.primary)
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                if let hotkey = currentPrompt.hotkey {
